...
const ProjectsPage = ({ user }) => {
  ...
  const monthlyProjects = baseProjects.filter(p => {
    ...
    // Include if the backend bucket matches the target month
    if (p.month && p.month === targetMonthStr) {
      return true;
    }

    // Include if it was delivered in the target month (handles YYYY-MM format)
    if (p.deliveredDate && p.deliveredDate.includes(targetMonthStr)) {
      return true;
    }

    const monthShort = MONTHS[viewMonth].slice(0, 3).toLowerCase();
    const fullMonth = MONTHS[viewMonth].toLowerCase();

    // Include if deliveredDate matches month text
    if (p.deliveredDate && (p.deliveredDate.toLowerCase().includes(monthShort) || p.deliveredDate.toLowerCase().includes(fullMonth))) {
        return true;
    }

    // Include if order date matches month text
    if (p.date && (p.date.toLowerCase().includes(monthShort) || p.date.toLowerCase().includes(fullMonth))) {
        return true;
    }
