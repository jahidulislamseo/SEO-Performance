            ...
            {sorted.map((m, i) => {
              ...
              return (
                <tr key={m.id || m.name} className={i < 3 ? 'lb-top' : ''}>
                  ...
                  <td><span style={{ color: '#10b981', fontWeight: 800, fontVariantNumeric: 'tabular-nums', whiteSpace: 'nowrap' }}>${(m.deliveredAmt || 0).toLocaleString()}</span></td>
                  <td><span style={{ color: '#f59e0b', fontWeight: 700, fontVariantNumeric: 'tabular-nums', whiteSpace: 'nowrap' }}>${(m.wipAmt || 0).toLocaleString()}</span></td>
                  <td>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
